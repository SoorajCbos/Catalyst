...
async function loginWithBackend(
  ...
  if (!response.ok) {
    let message =
      "Login failed. Check the username, password, or active status.";
...
function getEntryDescription(profile: UserProfile): string {
...
export default function Home() {
  ...
  async function handleSubmit(
    ...
    try {
      ...
      // The backend decides where the user must go. New users with
      // generated passwords are sent to /change-password first.
      window.location.assign(result.entryPoint);
    } catch (loginError) {
      setError(
        ...
        <div className="space-y-8">
          <div className="max-w-2xl space-y-5">
            ...
            <h1 className="text-4xl font-bold leading-tight text-[#172026] sm:text-5xl">
              One login directs every user to the correct
              organization workspace.
            </h1>

            <p className="max-w-xl text-base leading-7 text-[#52616b] sm:text-lg">
              The backend checks the user table by username,
              validates the password, confirms active status, and
              returns the user profile used for routing.
            </p>
          </div>
          ...
          <div className="grid gap-3 sm:grid-cols-3">
            {(
              ...
              <div
                key={profile}
                className="rounded-lg border border-[#d8cfbf] bg-white/70 p-4 shadow-sm"
              >
        ...
        <div className="rounded-lg border border-[#d6cab8] bg-white p-6 shadow-[0_24px_80px_rgba(54,41,24,0.14)]">
          ...
          <form className="space-y-4" onSubmit={handleSubmit}>
            <label className="block">
              <span className="text-sm font-semibold text-[#26323a]">
                Username
              </span>
          ...
          {loginResult ? (
            <div className="mt-6 rounded-lg border border-[#cbd7d3] bg-[#f4faf7] p-4">
              <div className="flex items-start justify-between gap-4">
                <div>
                  <p className="text-sm font-semibold text-[#1e3a3a]">
                    Authenticated user
                  </p>

                  <p className="mt-1 text-lg font-bold text-[#172026]">
              ...
              <dl className="mt-4 grid gap-3 text-sm">
                <div className="flex justify-between gap-4">
                  <dt className="text-[#63717a]">
                    Record ID
                  </dt>

                  <dd className="font-semibold text-[#26323a]">
                    {loginResult.user.recordId}
                  </dd>
                </div>
                ...
                <div className="flex justify-between gap-4">
                  ...
                  <dd className="font-semibold text-[#26323a]">
                    {loginResult.user.organizationId}
                  </dd>
                </div>
